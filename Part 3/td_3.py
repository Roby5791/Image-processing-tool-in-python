# -*- coding: utf-8 -*-
"""
Created on Tue Feb 16 12:37:56 2021
...
parser.add_argument("--blurryImage", help="Enter the output name of your image color for blurry image. Ex: blurry_couleur.bmp")
parser.add_argument("--pushbackImage", help="Enter the output name of your image color for pusch back image. Ex: pushback_couleur.bmp")
parser.add_argument("--colorizationImage", help="Enter the output name of your image color for colorization of image. Ex: gray_lena_couleur.bmp")
...
parser.add_argument("--sobelFilter", help="Enter the output name of your image color for the model of image recognition. Ex: recognition_lena_couleur.bmp")
...
#function for blurring image (filter)
def blurryImage(filename):
    f_lecture =open(filename,'rb') #read in binary mode
    f_ecriture =open(args.blurryImage,'wb') #read in binary mode
...
#function for push back (filter)
def pushbackImage(filename):
    f_lecture =open(filename,'rb') #read in binary mode
    f_ecriture =open(args.pushbackImage,'wb') #read in binary mode
...
#function to colorize Eugene ATGET image in grayscale
def colorization_image(filename):
   
    f_lecture =open(filename,'rb') #read in binary mode
    f_ecriture =open(args.colorizationImage,'wb') #read in binary mode
    i=1
    octet = bytes([0])
...
def gray_image(filename):
    ...
    f_lecture =open(filename,'rb') #read in binary mode
    f_ecriture =open("grey.bmp",'wb') #read in binary mode
    i=1
    octet = bytes([0])
...
def blurFilter(filename):
    f_lecture =open(filename,'rb') #read in binary mode
    f_ecriture =open("blur.bmp",'wb') #read in binary mode
...
#function to apply sobel filter for the model of image recognition
def imageRecognition(filename):
    f_lecture =open(filename,'rb') #read in binary mode
    f_ecriture =open(args.sobelFilter,'wb') #read in binary mode
    ...
    tri = trioctets.reshape((widthImage,heightImage,3))
    sobelFilter = tri
    #kernel of Sobel filter horizontaly
    kernelX = [[  1  , 0 ,   -1],
              [  2 ,   0 ,  -2 ],
              [  1  , 0 ,    -1 ]]
    #kernel of Sobel filter verticaly
    kernelY = [[ 1  , 2 ,   1],
              [  0 ,   0 ,  0],
              [  -1  , -2 ,  -1 ]]
    ...
    #convolution - Sobel filter horizontaly
    kernelCX = convolve(tri, kernelX)
    #convolution - Sobel filter verticaly
    kernelCY = convolve(tri, kernelY)

    for i in range(len(sobelFilter)):
        for j in range(len(sobelFilter)):
            #We square each pixel for convolution on horizontal (sobel)
            xx=np.square(kernelCX[i][j])
            #We square each pixel for convolution on vertical (sobel)
            yy=np.square(kernelCY[i][j])
            #We sum both (Gx*Gx + Gy*Gy)
            tot = xx + yy
            #We take the root sqrt(Gx*Gx + Gy*Gy). It means we considered the two composants of Sobel filter horizontaly and verticaly
            sobelFilter[i][j] = np.sqrt(tot)
    
    #To obtain the image for Sobel filter horizontaly, you can replace "sobelFilter" by kernelCX
    #To obtain the image for Sobel filter verticaly, you can replace "sobelFilter" by kernelCY
    #We print the sum of each convolution and normalize
    for i in sobelFilter:
       for j in i: 
           for k in j:
               #Normalization
               if (k <0): 
...
#-------------blurryImage
if (args.blurryImage and args.blurryImage[-3:] == "bmp"):
    blurryImage(filename) 
    print("The size of your image ", args.imageName, " is ", widthImage, " * ", heightImage, " .")
    visualize(args.blurryImage)
    
#-------------pushbackImage
if (args.pushbackImage and args.pushbackImage[-3:] == "bmp"):
    pushbackImage(filename) 
    print("The size of your image ", args.imageName, " is ", widthImage, " * ", heightImage, " .")
    visualize(args.pushbackImage)

#-------------colorizationImage
if (args.colorizationImage and args.colorizationImage[-3:] == "bmp"):
    colorization_image(filename) 
    visualize(args.colorizationImage)
...
#-------------imageRecognition
if (args.sobelFilter and args.sobelFilter[-3:] == "bmp"):
    gray_image(filename)
    blurFilter("grey.bmp")
    imageRecognition("blur.bmp") 
    print("The size of your image ", args.imageName, " is ", widthImage, " * ", heightImage, " .")
    visualize(args.sobelFilter)